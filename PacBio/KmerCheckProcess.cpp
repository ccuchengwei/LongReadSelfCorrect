// KmerCheckProcess.cpp - Check kmer correctness & distribution condition
//
...
KmerCheckResult KmerCheckProcess::process(const SequenceWorkItem& workItem)
{
	...
	*pAlignReader
	...
	delete pAlignReader;
	std::string target = (*m_params.pSeqMap)[key];
	...
	for(int i = m_params.size.first; i <= m_params.size.second; i++)
		scan(i, query, target, result);
	return result;
}
void KmerCheckProcess::scan(const int staticKmerSize, const std::string& query, const std::string& target, KmerCheckResult& result)
{
	int anchor = 0, range = (int)(staticKmerSize * 2.5f + 0.5f), section = (int)(staticKmerSize/3.f);
	for(int i = 0; i <= ((int)query.length() - staticKmerSize); i++)
	{
